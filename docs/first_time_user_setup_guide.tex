...
\author{First local setup for the FPGA board-based demo}
\date{Updated \today}
...
not as a paper. The expected use case is simple: prepare the FPGA board boot media,
...
under local repo folders such as \path{results/}, \path{output/}, and
\path{docs/}.

\section{1. Prepare the FPGA board SD Card First}
\subsection{Image to flash}
Start with the board, because the rest of the setup is easier to understand once
the target platform is clear. The FPGA board has QSPI boot firmware on the SOM and
uses the microSD card as the secondary boot device for Linux. For this project,
flash an official FPGA Ubuntu image onto the microSD card.

\begin{itemize}[leftmargin=*]
\item First choice for a fresh setup: \textbf{FPGA Ubuntu 24.04 LTS}.
\item Use Ubuntu 22.04 only if a specific FPGA app or tutorial you need still
targets 22.04.
...
\item Download the FPGA Ubuntu image for FPGA board/K26.
...
\begin{enumerate}[leftmargin=*]
\item Insert the flashed microSD card into the FPGA board carrier.
...
\subsection{Linux or FPGA board Ubuntu shell}
\begin{lstlisting}[style=cmd]
cd btp
...
\begin{lstlisting}[style=cmd]
python -c "import torch, torchvision, matplotlib; import p1_training.training_core; print('imports ok')"
\end{lstlisting}
...
Latest local smoke-run values to expect:
\begin{itemize}[leftmargin=*]
\item best run: \texttt{exp1\_hardware\_aware\_loss};
\item validation accuracy: 95.70\%;
\item spike sparsity: 56.48\%;
\item energy-proxy reduction vs baseline: 79.68\%;
\item hardware/Vivado stages: skipped in smoke mode.
\end{itemize}
...
\path{output/} & LaTeX report sources and compiled PDFs \\
...
Expected PDFs:
\begin{itemize}[leftmargin=*]
\item \path{output/first_time_user_setup_guide.pdf}
\item \path{output/progress_report.pdf}
\end{itemize}
...
FPGA board does not boot & Reflash the SD card, check serial output, and confirm boot firmware compatibility. \\
...
\begin{enumerate}[leftmargin=*]
\item FPGA board SD card is flashed and the board boots Ubuntu.
...
\item The setup guide and progress report PDFs are in \path{output/}.
\end{enumerate}

\section{References Used for Board Setup}
\begingroup
\small
The hardware validation flow is board-agnostic: it targets a generic FPGA
development board and does not depend on a specific vendor kit. For board
bring-up, use the documentation of your board vendor together with the
documentation of the FPGA toolchain you build with (for example, the Vivado
documentation when building with Vivado). See
\path{hardware_validation/fpga_board/README.md} for the batch flow itself.
\endgroup
