# FPGA board Hardware Validation Flow
...
4. Final board measurements (optional final stage) merged into the same schema.
...
```bash
python hardware_validation/fpga_board/scripts/run_hw_validation.py --model-id baseline_paper1 --scheduler-mode dense
```

Run both dense/event:

```bash
python hardware_validation/fpga_board/scripts/run_hw_validation.py --model-id baseline_paper1 --scheduler-mode both
```
