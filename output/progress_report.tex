...
layer, and LaTeX documentation. The latest completed run was a software smoke
run using \texttt{-SkipVivado -SkipHardware}. The main unfinished item is
physical validation on the board. Local xsim/Vivado evidence already
exists from earlier validation output, but fresh Vivado execution and board-level
...
\item Can the RTL and validation flow produce local evidence for the FPGA board path?
...
Validation & FPGA board-oriented xsim and Vivado batch scripts generate local logs, reports, and parsed JSON. \\
...
\begin{itemize}[leftmargin=*]
\item \path{p1_training/}: current SNN model, training core, baseline script,
and export utilities.
\item \path{p2_hardware_model/estimators.py}: hardware proxy functions.
...
to trace;
\item \path{hardware_validation/fpga_board/} was added for xsim regression, Vivado Tcl
...
The hardware validation path is local and FPGA board-oriented. It runs:
\begin{enumerate}[leftmargin=*]
\item xsim regression for dense and event scheduler modes;
\item Vivado synthesis/implementation through \path{hardware_validation/fpga_board/vivado/build.tcl};
...
For the latest completed smoke run, this stage was intentionally skipped. That
means the software and analysis results are fresh, while the hardware rows should
be treated as existing local validation evidence.
...
stronger hardware stimuli and real board measurements.
...
\item \path{output/progress_report.tex}: this report;
\item \path{output/final_paper.tex}: paper-style draft source.
\end{itemize}

\section{Open Work}
\begin{itemize}[leftmargin=*]
\item Run physical FPGA board measurements and merge them into the same hardware
metrics schema.
...
\section{Next Steps}
\begin{enumerate}[leftmargin=*]
\item Boot the FPGA board with the prepared FPGA Ubuntu SD card.
...
claim should wait for FPGA board physical measurements.
